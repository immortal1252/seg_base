...
def vmap(module: Callable, *args, **kwargs) -> torch.Tensor:
    batch_size = -1
    group_size = -1
    new_args = []
    for arg in args:
        batch_size, group_size, *_ = arg.shape
        new_v = E.rearrange(arg, "B S ... -> (B S) ...")
        new_args.append(new_v)

    for k in kwargs.keys():
        batch_size, group_size, *_ = kwargs[k].shape
        new_v = E.rearrange(kwargs[k], "B S ... -> (B S) ...")
        kwargs[k] = new_v

    grouped_output = module(*new_args, **kwargs)
    output = E.rearrange(
